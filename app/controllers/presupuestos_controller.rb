class PresupuestosController < ApplicationController
  ...
  before_action :logged_in_user, only: [:index, :show, :new, :edit, :create, :new, :update, :destroy, :presupuesto_reform ]
  ...
  def show
    if current_user.empresa == true
      ...
      if @habitacion != nil
        ...
        if (@budget = Budget.find_by(partida_id: @presupuesto.partida_id, user_id: current_user.id)) != nil
          @budget = Budget.find_by(partida_id: @presupuesto.partida_id, user_id: current_user.id)
        else
          @budget = Budget.new
        end 
      end       
  ...
  def presupuesto_reform
    ...
    if params[:empresa] != nil
      ...
      @presupuestos = Presupuesto.where(user_id: @empresa.id).where(reform_id: params[:id])
    else
      @presupuestos = Presupuesto.where(reform_id: params[:id]).where(user_id: current_user.id)
    end
